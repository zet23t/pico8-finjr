...
tile_heart = 16
tile_2x2_tree_big_1 = 103
tile_2x2_tree_big_2 = 105
tile_1x2_tree_big_1 = 101
tile_1x2_tree_big_2 = 102
...
local function path(stepsize, x1, y1, x2, y2, scatter, placecallback)
	local function halfit(x1,y1,x2,y2, n, dstart,dend)
		...
		local rx, ry = placecallback(x,y, d, dh, nx, ny, x1,y1,x2,y2)
		if rx then x, y = rx, ry end
...
function _init()
	...
	for i=1,#river_spawns / 10 do
		...
		local river_points = {}
		if not map_rivers[idx_map_screen(x,y)] then
			while true do
				...
				local h1,h2,h3,h4 = map_heights[idx_map_screen(x-1,y)] or 10, map_heights[idx_map_screen(x+1,y)] or 10,
					map_heights[idx_map_screen(x,y-1)] or 10, map_heights[idx_map_screen(x,y+1)] or 10
...
function draw_stone_house(x,y, w, h, blockindex)
	...
	for px=7, w - 5, 8 do
		l_spr(tile_1x1_reet_roof_middle,x + px, y - 6, ceil(min(8, w-5-px)/8))
	end
...
local function prepare_map(sectionx, sectiony)
	...
	local function connection(d, hcut, hoffset, x1,y1,x2,y2, ondraw)
		...
		path(d,x1,y1,x2,y2,.5, function(x,y,d,p, nx, ny,x1,y1,x2,y2)
--			assert(x1 and y1 and x2 and y2)
			...
			-- 	layer(5)
			-- end
			local rx, ry = ondraw(x,y,d, p,nx, ny,h,x1,y1,x2,y2)
			block(x,y)
			return rx, ry
		end)
	end
	...
	local function bridge(x1,y1,x2,y2)
		...
		add(bridgeDocPoints, {x1,y1})
		add(bridgeDocPoints, {x2,y2})
		...
		path(2.75,x1,y1,x2,y2,0, function(x,y, d, p, nx, ny)
			...
			for l=-6,6,2 do
				if l == -6 or l > -1 or abs(oy) < .5 then
	...
	local function get_intersecting_river_points(x1,y1,x2,y2, near_dist, nearby)
	...
	local road_sprite_points
	local function road(x1,y1,x2,y2,split)
		local nearby = get_intersecting_river_points(x1,y1,x2,y2, 14)
		...
		layer(4)
		l_pal()
		connection(5, 0, 0,x1,y1,x2,y2, function(x,y,d, p, nx, ny,h,x1,y1,x2,y2)
			if x1 then 
				::retry::
				assert(x1 and y1 and x2 and y2)
				local intersects = get_intersecting_river_points(x1,y1,x,y,8)--xyz
				get_intersecting_river_points(x2,y2,x,y,8, intersects)--xyz
				if #intersects > 0 then
					local mx,my = (x1+x2) * .5, (y1+y2) * .5
					local nx,ny = (mx+x) * .5, (my + y) * .5
					if distance(mx,my,nx,ny) > 1 then
						x,y = nx,ny
						goto retry
					end
				end
			...
			add(road_sprite_points, {x=x,y=y,nx=nx,ny=ny})
			return x, y
		end)
	end
	...
	local map_idx = idx_map_screen(sectionx, sectiony)
	local roads = map_roads[map_idx]
	if roads then
		::retry::
		...
		for p in all(river_points) do
			...
			local d= dx*dx+dy*dy
			if d < 25*25 then goto retry end
		end
		layer(150)
		-- l_call(function()rectfill(cx-2,cy-2,cx+2,cy+2,9) end)
		srand(sectionx * 4 + (sectiony - .5) * 10)
		if map_roads[idx_map_screen(sectionx,sectiony-1)] then
		...
		if map_roads[idx_map_screen(sectionx-1,sectiony)] then
	...
	-- draw_stone_wall(50,25,36 + rnd(10),20+rnd(10), block)
		for i=1,rnd(5) do
			draw_stone_house(14+rnd(100),14+rnd(100),16 + rnd(20),10 + rnd(10), block)
		end
		...
		for y=0,120,8 do
			...
			if h < 0 then
				...
				if not isblocked(spx, spy) then
					...
					elseif rnd() > pn + .3 and pn < 0.1 and pn > -.25 then
						-- trees
						if rnd() > .7 then
							l_spr(97, spx, spy)
							l_spr(99, spx, spy - 4)
						elseif not isblocked(spx, spy + 8) then
							l_spr(115, spx, spy)
							if rnd() > .3 then
								if rnd() > .5 then
									l_spr(tile_1x2_tree_big_1, spx, spy - 8, 1, 2)
								else 
									l_spr(tile_1x2_tree_big_2, spx, spy - 8, 1, 2)
								end
							elseif rnd() > .5 then
								l_spr(tile_2x2_tree_big_1, spx-4, spy - 8, 2, 2)
							else
								l_spr(tile_2x2_tree_big_2, spx-4, spy - 8, 2, 2)
							end
						end
...
--	layer(2)
	...
	l_spr(1, ppx+1, ppy, 1, 1, right)
	l_spr(1, ppx-1, ppy, 1, 1, right)
	l_spr(1, ppx, ppy-1, 1, 1, right)
	l_pal()
...
local fade, lerp, grad

-- return range: [-1, 1]
function perlin:noise(x, y, z)
	y = y or 0
	z = z or 0
	local flrx,flry,flrz = flr(x), flr(y), flr(z)
	-- calculate the "unit cube" that the point asked will be located in
	local xi = band(flrx,255)
	local yi = band(flry,255)
	local zi = band(flrz,255)
...
__gfx__

...

...

...
__sfx__
010e00000e4430e4030e4430f4030e44310403164030e4030e4430e4030e4030c4030e443024030e443024030e4431140313403114030e443164030c4030c4030e4430c4030e4430c4030e4430c4030c4030c403
